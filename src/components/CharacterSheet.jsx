...
const createDefaultCharacter = (player) => ({
  ...
  level: 1, body: 1, spirit: 1, mind: 1, lr: 2, os: 0, armor: 0,
  aspects: [], wounds: { light: [], medium: [], heavy: [] },
  abilities: ['Усиление', 'Возможность']
})

export default function CharacterSheet({ roomId, userId, isMaster, selectedUid, selectedPlayer, onCharacterNameChange }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(charRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        const defaultCharacter = createDefaultCharacter(selectedPlayerRef.current)
  ...
  const handleSave = async () => {
    ...
    await updateDoc(doc(db, 'rooms', roomId, 'characters', selectedUid), savedCharacter)
  ...
  if (!selectedUid && isMaster) {
    return <div className="bg-gray-800/80 backdrop-blur p-6 rounded-2xl border border-purple-500/20 shadow-xl text-gray-300">В комнате пока нет игроков. Их персонажи появятся здесь после присоединения.</div>
  }
    ...
    <div className="bg-gray-800/80 backdrop-blur p-6 rounded-2xl border border-purple-500/20 shadow-xl">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          {!isEditing && <button onClick={() => setIsCollapsed(!isCollapsed)} className="px-3 py-1 rounded-lg bg-gray-700 hover:bg-gray-600 text-white transition">{isCollapsed ? 'Развернуть' : 'Свернуть'}</button>}
          {isEditing ? (
            <button onClick={cancelEditing} className="px-4 py-1 rounded-lg text-white transition bg-gray-600 hover:bg-gray-500">Отмена</button>
          ) : (
            <button onClick={startEditing} className={`px-4 py-1 rounded-lg text-white transition ${canEdit ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-600 cursor-not-allowed'}`} disabled={!canEdit}>Редактировать</button>
          )}
        </div>
      </div>
        ...
        <div className="mt-4">
          {isEditing ? (
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-4"><span>ЛР: {character.lr}</span><span>ОС: {character.os}</span><span>Броня: {character.armor}</span></div>
              <div><p className="font-semibold">Аспекты:</p><ul className="list-disc list-inside text-sm">{(character.aspects || []).map((aspect, index) => <li key={index}>{aspect}</li>)}</ul></div>
              <div><p className="font-semibold">Травмы:</p><div className="text-sm"><span className="text-green-400">Лёгкие: {wounds.light?.length || 0}</span><span className="text-yellow-400 ml-3">Средние: {wounds.medium?.length || 0}</span><span className="text-red-400 ml-3">Тяжёлые: {wounds.heavy?.length || 0}</span></div></div>
            </div>
          )}
        </div>
      )}
